...
from ratchet import logsetup  # noqa: E402
from ratchet import tray  # noqa: E402
...
def test_logging_to_a_file_writes_ratchet_lines(tmp_path):
    path = tmp_path / "sub" / "ratchet.log"
    logsetup.configure(path)
    logging.getLogger("ratchet").info("hello from the test")
...
def test_logging_does_not_double_up_when_configured_twice(tmp_path):
    ...
    logsetup.configure(path)
    logsetup.configure(path)
    logging.getLogger("ratchet").info("only once")
    for h in logging.getLogger().handlers:
        h.flush()
...
def test_third_party_chatter_stays_out_of_the_log(tmp_path):
    ...
    logging.getLogger("ratchet").warning("something real")
    for h in logging.getLogger().handlers:
        h.flush()
