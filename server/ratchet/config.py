...
def _default_data_dir() -> Path:
    ...
    if sys.platform == "win32":
        # Capitalised to match the product name, as Windows app-data folders
        # conventionally are; lower-case under ~/.local/share, as those are.
        return Path(os.environ.get("LOCALAPPDATA", "~/AppData/Local")) / "Ratchet"
    return Path("~/.local/share/ratchet")
...
class Config:
    ...
    def db_path(self) -> Path:
        return self.data_dir / "ratchet.sqlite3"
...
_ENV_OVERRIDES = [
    ("RATCHET_AUTH_TOKEN", "service", "auth_token"),
    ("RATCHET_CALIBRE_USERNAME", "calibre", "username"),
    ("RATCHET_CALIBRE_PASSWORD", "calibre", "password"),
]
