"""Ratchet HTTP service.

Run:  RATCHET_CONFIG=/path/to/config.toml uvicorn ratchet.main:app --host ... --port ...
(or just `python -m ratchet` — see __main__.py — which reads host/port from config)
...
CONFIG_PATH = os.environ.get("RATCHET_CONFIG", "config.toml")
...
app = FastAPI(title="Ratchet", version="0.1")
...
def _fetch_epub_to_temp(library_id: str, book_id: int) -> tuple[str, tempfile.TemporaryDirectory]:
    tmp = tempfile.TemporaryDirectory(prefix=f"ratchet-{book_id}-")
    ...
    if not chapters:
        raise HTTPException(
            422, "no chapters with embedded chapterurl found in the epub — "
                 "it wasn't produced by FanFicFare, so Ratchet can't reason "
...
def libraries() -> dict:
    """Every library on the content server, plus which one Ratchet defaults to."""
    try:
...
def add_book(url: str = Body(embed=True), library: str | None = LIB_Q) -> dict:
    ...
    duplicate guards: Ratchet's own records by story URL, and calibre's
    ...
    if existing is not None:
        ...
        # Ratchet. Confirm the book is really still there before refusing,
    ...
    with tempfile.TemporaryDirectory(prefix="ratchet_add_") as tmp:
