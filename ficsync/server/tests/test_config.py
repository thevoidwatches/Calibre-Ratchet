...
def test_env_file_overrides_toml(tmp_path):
    ...
    (tmp_path / ".env").write_text(
        'FICSYNC_CALIBRE_USERNAME=bob\n'
        'FICSYNC_CALIBRE_PASSWORD="p w"\n'
        '# comment\n\nnot a kv line\n',
        encoding="utf-8",
    )
...
def test_process_env_beats_env_file(tmp_path, monkeypatch):
    cfg_path = _write_config(tmp_path)
    (tmp_path / ".env").write_text("FICSYNC_CALIBRE_PASSWORD=filepw\n", encoding="utf-8")
    monkeypatch.setenv("FICSYNC_CALIBRE_PASSWORD", "envpw")
    cfg = load_config(cfg_path)
...
def test_resolve_host_passes_literals_through():
    from ficsync.config import resolve_host
